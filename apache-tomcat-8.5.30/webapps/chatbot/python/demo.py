...
def train():
    ...
    config = tf.ConfigProto(allow_soft_placement=True, device_count={'cpu':7})
...
#        for step in xrange(30000):
            step += 1
            sample_encoder_inputs, sample_decoder_inputs, sample_target_weights = get_samples(train_set, 1083)
            input_feed = {}
            ...
            if step % 10 == 0:
                ...
                if len(previous_losses) > 10 and loss_ret > max(previous_losses[-10:]) and learning_rate.eval() >= 0.0001:
