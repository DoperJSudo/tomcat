...
max_token_id = wordToken.load_file_list(['/root/apache-tomcat-8.5.30/webapps/chatbot/python/samples/question', '/root/apache-tomcat-8.5.30/webapps/chatbot/python/samples/answer'], min_freq)
...
def get_train_set():
    ...
    train_set = []
    with open('/root/apache-tomcat-8.5.30/webapps/chatbot/python/samples/question', 'r') as question_file:
        with open('/root/apache-tomcat-8.5.30/webapps/chatbot/python/samples/answer', 'r') as answer_file:
            while True:
...
def main(ques):
    with tf.Session() as sess:
        ...
        saver.restore(sess, '/root/apache-tomcat-8.5.30/webapps/chatbot/python/model/demo')
    #    sys.stdout.write("> ")
        sys.stdout.flush()
    ...
    #    while input_seq:
        if input_seq:
            ...
            if (len(input_id_list)):
                ...
                return re.sub('\s', '', " ".join(outputs_seq))
            else:
                return "WARN：词汇不在服务区"
